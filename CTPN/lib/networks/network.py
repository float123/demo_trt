...
class Network(object):
    ...
    def Bilstm(self, input, d_i, d_h, d_o, name, trainable=True):
        ...
        with tf.variable_scope(name) as scope:
            ...
            img.set_shape([None, None, d_i])  # d_i=512, [batch_szie, max_time, depth]
            ...
            lstm_out = tf.concat(lstm_out, axis=-1)  # [max_len,batch_size,2*hidden_num]
    ...
    def lstm_fc(self, input, d_i, d_o, name, trainable=True):
        with tf.variable_scope(name) as scope:
            ...
            N, H, W, C = shape[0], shape[1], shape[2], shape[3]
            input = tf.reshape(input, [N*H*W,C])  # [ N*H*W,  C ]
    ...
    def spatial_softmax(self, input, name):
        input_shape = tf.shape(input)  # (1, 37, 750, 2)
        # d = input.get_shape()[-1]
...
# d_i = 512, d_h = 128
def Bilstm(self, input, d_i, d_h, name):
    img = input
    with tf.variable_scope(name) as scope:
        shape = tf.shape(img)
        N, H, W, C = shape[0], shape[1], shape[2], shape[3]
        img = tf.reshape(img, [N * H, W, C])
        img.set_shape([None, None, d_i])  # [batch_szie, max_time, ebedding]:[37, 75, 512]
