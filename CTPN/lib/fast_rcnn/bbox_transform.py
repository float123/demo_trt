...
def bbox_transform(ex_rois, gt_rois):
    ...
    """
    ex_widths = np.maximum(ex_rois[:, 2] - ex_rois[:, 0] + 1.0, 1)
    ex_heights = np.maximum(ex_rois[:, 3] - ex_rois[:, 1] + 1.0, 1)
    ...
    gt_widths = np.maximum(gt_rois[:, 2] - gt_rois[:, 0] + 1.0, 1)
    gt_heights = np.maximum(gt_rois[:, 3] - gt_rois[:, 1] + 1.0, 1)
